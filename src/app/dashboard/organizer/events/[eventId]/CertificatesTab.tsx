    ...
    if (certificates.length === 0) {
        return (
            <EmptyState
                ...
                description={
                    language === 'ar'
                        ? 'لم يتم إصدار أي شهادات لهذا الحدث بعد'
        ...
        <div className="space-y-6">
            ...
            <LiquidGlassCard blurIntensity="md" interactive={false} className="p-6">
                <h3 className="text-lg font-semibold text-[var(--label)] mb-4 flex items-center gap-2">
                    ...
                    {language === 'ar' ? 'ملخص الشهادات' : 'Certificates Summary'}
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                        <div className="text-sm text-[var(--secondary-label)]">
                            {language === 'ar' ? 'عدد الممارسين الصحيين' : 'HCPs with Certificates'}
                        </div>
                        <div className="text-2xl font-bold text-[var(--label)]">
                            {hcpIds.size}
                        </div>
            ...
            <LiquidGlassCard blurIntensity="md" interactive={false} className="p-6">
                <h3 className="text-lg font-semibold text-[var(--label)] mb-4 flex items-center gap-2">
                    ...
                    {language === 'ar' ? 'الشهادات الصادرة' : 'Issued Certificates'}
                </h3>
                        ...
                        >
                            <div className="flex-1">
                                <div className="text-sm font-medium text-[var(--label)]">
                                    {language === 'ar' ? 'شهادة رقم' : 'Certificate'} {cert.id.slice(-8)}
                                </div>
                                <div className="text-xs text-[var(--secondary-label)] mt-1">
                                    {language === 'ar' ? 'صادرة لـ' : 'Issued to'} HCP: {cert.hcpId}
                                </div>
                                {cert.issuedAt && (
                                    <div className="text-xs text-[var(--secondary-label)] mt-1">
