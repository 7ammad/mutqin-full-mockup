...
import { BarChart3, TrendingUp, Users, AlertCircle, MapPin, Calendar, CheckCircle2 } from "lucide-react";
...
export default function EventManagerAnalyticsTab({ assignments, language }: EventManagerAnalyticsTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Attendance Rate by City */}
        <LiquidGlassCard blurIntensity="lg" interactive={false} className="p-6">
          <div className="flex items-center gap-2 mb-4">
            <MapPin className="h-5 w-5 text-[var(--apple-blue)]" />
                ...
                return (
                  <tr key={event.id} className="border-b border-[var(--system-fill)] hover:bg-[var(--system-fill)]/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-center">
                      <Badge
                        variant={rate >= 80 ? "default" : rate >= 50 ? "outline" : "outline"}
                        className={
                          rate >= 80
